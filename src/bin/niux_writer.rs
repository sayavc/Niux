...
fn create_autogen(config_path: &str, hook_config_path: &str) -> anyhow::Result<()> {
    ...
    fs::create_dir_all(config_dir)
        .with_context(|| format!("Failed to create dir: {}", config_dir.display()))?;
    let content = format!(
        include_str!("../assets/autogen_config.kdl"),
        config_path, hook_config_path
    );
    let path_to_write = config_dir.join("niux_autogen.kdl");
    fs::write(&path_to_write, content)
        .with_context(|| format!("Failed to write config dir: {}", path_to_write.display()))?;
    Ok(())
}
