...
> [!NOTE]
> *"1000+ строк кода на Rust, которые можно было бы эффективно заменить одним баш-скриптом."*
> — **Майнтейнер awesome-nix**

## Зачем Niux?

Управление пакетами `home-manager` и `NixOS` вручную требует постоянного открытия конфигурационных файлов, добавления строк в списки пакетов и запуска команд пересборки системы.

**Niux** автоматизирует этот рабочий процесс: он предоставляет легковесный CLI для установки, удаления и управления пакетами с помощью коротких интуитивных команд — аналогично традиционным менеджерам пакетов вроде `apt` или `pacman`, — строго сохраняя при этом декларативную природу Nix.
...
- **Обновление флейков, просмотр диффов генераций через интеграцию с `nvd` и очистка системы**
